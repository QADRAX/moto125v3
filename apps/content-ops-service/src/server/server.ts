...
export function createServer(opts: ServerOptions) {
  ...
  app.get(ROUTES.LOGS_STREAM, (req, res) => {
    ...
    for (const e of opts.bus.snapshot()) res.write(`data: ${JSON.stringify(e)}\n\n`);
    const unsub = opts.bus.subscribe((e) => res.write(`data: ${JSON.stringify(e)}\n\n`));
    req.on("close", () => { unsub(); res.end(); });
  });

  // Static UI (si tienes UI build)
  const candidate = opts.uiStaticDir && fs.existsSync(opts.uiStaticDir)
    ? opts.uiStaticDir
    : path.join(__dirname, "static");
  opts.log.info(`Serving UI from: ${candidate}`);
