...
import type { MotoType, Moto } from "@moto125/api-client";
import { mediaUrl, slugify } from "@/utils/utils";
import { getMirrorState } from "@/server/dataMirror";

export default async function TypeCard({
  type,
  classSlug,
}: {
  type: MotoType;
  ...
  // Count motos belonging to this type
  const count = state.data.motos.reduce((acc: number, m: Moto) => {
    const typeId = m.motoType?.documentId;
    return acc + (typeId === type.documentId ? 1 : 0);
  }, 0);

  const href = `/motos/${classSlug}/${slugify(type.name ?? "")}`;
  const imgSrc = mediaUrl(type.image?.url);
  ...
  return (
    <Link
      ...
      aria-label={`${type.fullName ?? type.name} — ${renderCount(count)}`}
    >
      ...
      <div className="p-4">
        ...
        <p className="mt-0.5 text-sm opacity-70">Ver motos</p>
      </div>
    </Link>
  );
}
